...
/**
 * Generates a complete rotation plan for a game that ensures fair play time distribution
 * @param input - Configuration for the rotation plan
 * @returns Created GamePlan with all PlannedRotations
 */
export async function generateRotationPlan(input: RotationPlanInput): Promise<GamePlan> {
  const {
    gameId,
    halfLengthMinutes,
  ...
  // Track who's on field for each rotation
  const fieldState: Map<number, Set<string>> = new Map();
  
  // Initialize with starting lineup
  const startingPlayerIds = new Set(startingLineup.map(s => s.playerId));
  fieldState.set(0, startingPlayerIds);

  // Calculate fair rotation distribution
  const rotationSchedule = calculateFairRotations(
    availablePlayers,
    startingLineup,
    totalRotations,
    rotationsPerHalf,
    input.maxPlayersOnField
  );

  // Create planned rotations
  const rotationPromises = rotationSchedule.map(async (rotation, index) => {
    ...
    return client.models.PlannedRotation.create({
      ...
      gameMinute,
      half,
      plannedSubstitutions: JSON.stringify(rotation.substitutions),
      coaches,
    });
  });
  ...
  console.log(`Created ${rotationSchedule.length} planned rotations`);
...
/**
 * Gets rotations affected by a player availability change
 * @param rotations - All planned rotations
 * @param playerId - Player whose availability changed
 * @returns Rotation numbers that need attention
 */
export function getAffectedRotations(
  rotations: PlannedRotation[],
  playerId: string
): number[] {
  ...
  rotations.forEach(rotation => {
    const subs: PlannedSubstitution[] = JSON.parse(rotation.plannedSubstitutions as string);
    const isInvolved = subs.some(
      sub => sub.playerOutId === playerId || sub.playerInId === playerId
    );
    
    if (isInvolved) {
      affected.push(rotation.rotationNumber);
    }
  });
...
/**
 * Suggests replacement players for an unavailable player
 * @param unavailablePlayerId - Player who is unavailable
 * @param availablePlayers - Roster of available players
 * @param currentRotations - Current rotation plan
 * @param rotationNumber - Which rotation to suggest for
 * @returns Suggested player IDs sorted by play time (least to most)
 */
export function suggestReplacements(
  unavailablePlayerId: string,
  availablePlayers: SimpleRoster[],
  currentRotations: PlannedRotation[],
  rotationNumber: number
): string[] {
  // Calculate current play time up to this rotation
  const playTime = new Map<string, number>();
  availablePlayers.forEach(p => playTime.set(p.playerId, 0));
  
  // Track field state through rotations
  const rotationsUpToNow = currentRotations.filter(r => r.rotationNumber < rotationNumber);
  const currentField = new Set<string>();
  
  rotationsUpToNow.forEach(rotation => {
    const subs: PlannedSubstitution[] = JSON.parse(rotation.plannedSubstitutions as string);
    subs.forEach(sub => {
      currentField.delete(sub.playerOutId);
      currentField.add(sub.playerInId);
      playTime.set(sub.playerInId, (playTime.get(sub.playerInId) || 0) + 1);
    });
  });
  
  // Find players not currently on field (excluding unavailable player)
  const benchPlayers = availablePlayers
    .filter(p => p.playerId !== unavailablePlayerId && !currentField.has(p.playerId))
    .map(p => ({
      playerId: p.playerId,
      time: playTime.get(p.playerId) || 0,
    }))
    .sort((a, b) => a.time - b.time);
  
  return benchPlayers.map(p => p.playerId);
}
