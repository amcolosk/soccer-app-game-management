  ...
  it('continues cleanup when one safe game delete fails', async () => {
    const client = {
      models: {
        ...
        Shot: { list: vi.fn().mockResolvedValue({ data: [], nextToken: null }), delete: vi.fn().mockResolvedValue({}) },
        Save: { list: vi.fn().mockResolvedValue({ data: [], nextToken: null }), delete: vi.fn().mockResolvedValue({}) },
