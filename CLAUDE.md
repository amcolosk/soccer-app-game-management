...
**Guest-auth exception (Fan Mode, Milestone B1):** `getFanGameView` is the first genuinely public/unauthenticated operation in the app — it carries `allow.guest()` **and** `allow.authenticated('identityPool')` (the Identity Pool's IAM auth mode, a different "authenticated" than the Cognito user-pool one everywhere else) instead of `allow.ownersDefinedIn('coaches')`. It's reachable via a public `/watch/:token` link with zero Cognito session. The underlying `ShareLink`/`FanViewRateLimit` models stay fully closed (`allow.authenticated().to([])`, same as `CalendarFeed`) — no client, guest or coach, ever reads/writes them directly; only `getFanGameView`'s Lambda touches them, via a shared token→team→validity→rate-limit pipeline (`amplify/functions/shared/shareLinkAccess.ts`). Every other model/operation is untouched by this exception.
