...
**Guest-auth exception (Fan Mode + Sideline Stat Tracker, Milestones B1/B2):** `getFanGameView` was the first genuinely public/unauthenticated operation in the app; `getStatTrackerView` and `submitStatEvent` (Milestone B2) followed the same pattern — all three carry `allow.guest()` **and** `allow.authenticated('identityPool')` (the Identity Pool's IAM auth mode, a different "authenticated" than the Cognito user-pool one everywhere else) instead of `allow.ownersDefinedIn('coaches')`. They're reachable via public `/watch/:token` (read-only) and `/track/:token` (write) links with zero Cognito session. `submitStatEvent` is this app's first unauthenticated **write** path: it writes `Goal`/`Shot`/`Save` records through a real AppSync mutation (`generateClient<Schema>({ authMode: 'iam' })` inside the Lambda, not a raw DynamoDB SDK write — the latter would never fire the coach's live `observeQuery` subscription, per the documented hazard at `src/components/Home.tsx:135-145`). This requires a schema-level `.authorization((allow) => [allow.resource(submitStatEvent).to(['mutate'])])` grant on `a.schema({...})` in `amplify/data/resource.ts` — **schema-wide and mutate-verb-wide by construction** (this Amplify version's `allow.resource()` has no per-model or per-CRUD-verb scoping at any level), so this one Lambda's execution role can technically create/update/delete on every model. The real narrowing is `submitStatEvent`'s own fixed, reviewed handler code (its `.create()` call sites on `Goal`/`Shot`/`Save` only, chosen by hardcoded branches the public API surface can't influence), not IAM. The underlying `ShareLink`/`FanViewRateLimit` models stay fully closed (`allow.authenticated().to([])`, same as `CalendarFeed`) — no client, guest or coach, ever reads/writes them directly; only the three Lambdas above touch them, via a shared token→team→validity→rate-limit→game-selection pipeline (`amplify/functions/shared/shareLinkAccess.ts`), which takes a `dimension: 'read' | 'write'` parameter so a helper's write-tapping and a fan's read-polling of the same team never share a rate-limit budget. Every other model/operation is untouched by this exception.
...
Current game time = `elapsedSeconds + (now - lastStartTime)` when running; `elapsedSeconds` alone when `lastStartTime` is null (paused). Auto-pauses at `halfLengthMinutes * 60`. Any code touching timer/halftime/rotation logic needs to reason in game-clock seconds, not wall-clock time — the actual conversion formula lives in `src/utils/gameClock.ts` (extracted from `useGameSubscriptions.ts` in Milestone B1, now also used by the public `FanGameView`); `gameTimeUtils.ts` is display-formatting only (MM:SS strings, datetime-local conversion), not clock arithmetic; `gameCalculations.ts` holds score/half-detection logic. **Milestone B2 adds a Lambda-side mirror** at `amplify/functions/shared/gameClock.ts` — a Lambda can't import from `src/`, so `submitStatEvent` derives a helper-submitted event's `gameSeconds`/`half` server-side from this parity-tested duplicate (never trusted from the untrusted public client); keep both copies in sync on any change, per `gameClock.test.ts`'s own parity test in that directory. The same pure-module-plus-parity-test pattern is used for goalkeeper attribution: `src/utils/playTimeCalculations.ts`'s `getCurrentGoalkeeperId` and its Lambda-side twin `amplify/functions/shared/goalkeeper.ts`'s `computeActiveGoalkeeperId`, parity-tested in `goalkeeper.test.ts` — keep both in sync on any change to GOALKEEPER-role derivation semantics. Same pattern again for live score derivation: `gameCalculations.ts`'s `computeScoreFromGoals` (the source of truth for "what is the score right now" while a game is active — `Game.ourScore`/`opponentScore` in the DB is only written at creation and completion, never mid-game) and its Lambda-side twin `amplify/functions/shared/score.ts`'s `computeScoreFromGoals`, parity-tested in `score.test.ts`; `score.ts` also exports `resolveScore`, the shared LIVE-vs-persisted branch policy both `get-fan-game-view` and `get-stat-tracker-view` call.
