...
- State the risk tier for this change at the top of your report (per the `dev-pipeline` skill's Risk tiers section) and why — the orchestrating thread's `npm run classify:risk-tier` output, if it gave you one, is authoritative for Tier 2; use your own judgment only to decide between Tier 0 and Tier 1.
