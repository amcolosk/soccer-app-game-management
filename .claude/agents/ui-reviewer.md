...
- **The check runs both directions.** As well as verifying the code matches the spec, verify the spec still matches the code: when reviewing an implementation that adds/changes a screen, component, or interaction pattern, confirm `docs/specs/UI-SPEC.md` was actually updated to describe it (new subsection under §7 Screen Specs, an addition to §8 Modal Patterns, a new row in §5.7 Status Badges, etc., as applicable) — not just that the plan said it would be. A shipped UI change with no corresponding spec update is a real finding (Minor for an omission, Major if the spec now actively contradicts what's shipped), independent of whatever else you find in the browser.
