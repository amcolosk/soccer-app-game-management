...
|---|---|---|
| none (no match at any precedence step), `gameDate` in the past | active | **skip** (no write) — counted in `skippedCount` with a warning ("the game date has already passed"). Fixed post-launch (issue #188): a feed's rolling window otherwise imported every already-played event as a fresh `scheduled` game. Applies only to genuinely-new rows — a past-dated event that matches an existing game via step 1 or step 2 still updates/adopts/cancels it normally. |
| none (no match at any precedence step), `gameDate` missing or not in the past | active | **create** — id = deterministic hash (Major 4), `ConditionExpression: attribute_not_exists(id)`, `coaches` from team, `status: 'scheduled'` |
