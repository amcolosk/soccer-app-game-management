...
**Implementation status: done** (commits `6b3e9f5`, `371f9fe` on `claude/app-architecture-eval-pcfpj3`). `gate:commit` green. A `validation-reviewer` pass caught a real Major gap the plan and its own review didn't: `useOfflineMutations.ts` has a *second*, independently-mounted drain path (`executeSingleMutation`) with the identical silent-error-swallow bug fixed in `useOfflineQueueDrain.ts` — both hooks race to drain the same queue on reconnect (`AppLayout.tsx`), so the untouched path could still have silently dropped a failed `PlayTimeRecord.update` replay. Fixed and re-verified in the second commit.
...
### Implementation status: **done**, with one design gap found and closed during implementation

The written design above (v4) was still incomplete: it never considered that the exact same "large gap, `isRunning` locally false" code path fires on a completely normal, very common case — **a second coach opening an already-running game for the first time**, or simply this session's first page load. Nothing in the v4 design distinguished that from "this device's timer was running and then lost continuity" (a real crash/force-quit). Implementing the modal as designed would have nagged every second coach who opens the app mid-half, likely making it the single most-complained-about part of the app. Neither architect-review round caught this — it only surfaces when you trace which code path actually executes on a fresh mount versus a genuine backgrounding-and-recovery.

**Fix, not in any prior version of this plan:** a per-user, per-game `localStorage` heartbeat (`src/constants/gameTimer.ts`, `buildTimerHeartbeatStorageKey(userId, gameId)`), written by `useGameTimer.ts` whenever `isRunning` transitions to `true` on a device. Its *presence* is required before the gap-confirmation logic engages at all — a device that has never had this game's timer running locally can never see the modal, regardless of how large the computed gap is, exactly matching pre-existing silent behavior for that case. Presence-only (not recency) is sufficient: the only way `isRunningRef.current` is `false` while the DB says `status: in-progress` is either a genuine fresh mount on this device (crash/reload/first-time-open) or a different device entirely — normal continuous running on the same device never reaches this branch at all (an earlier guard returns first). Scoped by user id, not just game id, so a shared device signed into a different coach's account doesn't inherit another coach's continuity signal.

**Also simplified from the written design:** no new modal component was built. The app already has a generic `useConfirm()`/`ConfirmModal` (title/message/two-button, with accessible focus management and Escape-to-cancel already built in) used elsewhere in this exact file for other confirmations (e.g. "End First Half?"). The gap-confirmation dialog reuses it directly — Escape/dismiss already resolves to "false," which naturally satisfies the "default to needs review" edge case with no extra code. This removed "New confirmation modal component" from the files below.

**"Adjust" resolution simplified too:** rejecting the proposed gap doesn't need any correction UI. It applies nothing (`currentTime`/`isRunning` stay at their prior values) and sets `manuallyPausedRef`, so the coach's *existing* Resume button (`handleResumeTimer`) is the natural next action — it already re-anchors from whatever `currentTime` currently is.

**Issue B/C sequencing (open question #7) resolved by landing B alone first**, in the current (un-refactored) `useGameSubscriptions.ts`, with care taken not to disturb the existing guards (`manuallyPausedRef`, `isRunningRef`, `gameStateRef`, the stale-event checks, the `:143`/`:199` score-preservation asymmetry). Issue C's eventual refactor must now also account for the new gap-detection branch and `pendingGapCorrection` state as part of its characterization-test net, not just the ~10 refs originally inventoried.

### Files (as actually changed)
- `src/constants/gameTimer.ts` (new) — `MAX_GAME_SECONDS`, `ANOMALOUS_GAP_THRESHOLD_SECONDS` (600s), `buildTimerHeartbeatStorageKey`
- `src/components/GameManagement/hooks/useGameTimer.ts` — heartbeat write on `isRunning` → true; replaced the hardcoded `7200` with `MAX_GAME_SECONDS`
- `src/components/GameManagement/hooks/useGameSubscriptions.ts` — gap detection + `pendingGapCorrection`/`resolveGapCorrection`, at the resume-jump computation site, gated by heartbeat presence
- `src/components/GameManagement/GameManagement.tsx` — wiring via the existing `useConfirm()`, no new component
- `src/utils/analytics.ts` — `TIMER_GAP_ACCEPTED`/`TIMER_GAP_ADJUSTED` events
- `docs/specs/Game-Management-Spec.md` — new §3.6; §3.4/§3.5 updated to cross-reference it and to reflect Issue A's `closeAllOpenPlayTimeRecords` primary/backstop split (was stale after that change too)
- `docs/specs/UI-SPEC.md` — new subsection under §7.4 documenting the dialog copy/behavior
- Tests: 11 new cases in `useGameTimer.test.ts`/`useGameSubscriptions.test.ts` covering the heartbeat-absence silent path, the below-threshold silent path, the pending-correction path, both boundary exclusions, dedup-while-pending, and both `resolveGapCorrection` outcomes; 3 new cases in `GameManagement.test.tsx` for the confirm-dialog wiring. **E2E regression coverage (Issue F6) remains outstanding** — this environment has no deployed Amplify backend to run Playwright against; needs verification via CI or a local run before merge.

### Edge cases
- A gap in the second half large enough to feel "anomalous" but nowhere near `MAX_GAME_SECONDS` is the primary case this issue exists for — covered by test.
- Coach dismisses the modal without choosing — resolves to "adjust" (false), not either extreme, for free via `useConfirm()`'s existing Escape handling.
- A second gap proposal is never raised while one is already pending (deduped via a ref, since the `observeQuery` effect only depends on `[game.id]`).

---

## Issue C — Consolidate the ref-guard race-patching pattern

**Implementation status: done.** By the time this was implemented, Issue B had already grown the guard inventory further — it added `pendingGapCorrectionRef` and `userIdRef` to `useGameSubscriptions.ts`, so the "~10 refs" below is now closer to a dozen; the inventory and doc block written during this implementation reflect the current count, not the plan's original one. The refactor followed the revised-fix guidance exactly: three pure, order-independent decision functions were extracted (`classifyIncomingGameEvent`, `mergeIncomingGameState`, `computeGapConfirmationDecision`) preserving their distinct inputs rather than collapsing to one predicate; the ordering-sensitive early-return sequence (completed-status short-circuit → stale-event checks → `manuallyPausedRef` reset → `isRunningRef` check) was left untouched inline, exactly as flagged as the risky part not to touch. Two characterization-test gaps the plan called out by name were filled *before* the extraction (the `completed`-branch's non-preservation of score — the other side of the #177 asymmetry — and the confirmed-pause `manuallyPausedRef` reset), and the full existing suite (34 tests in this file alone) passed unchanged before and after. `CLAUDE.md` was deliberately not touched, per the plan's explicit human-sign-off requirement — the new guard-inventory table lives in `docs/ARCHITECTURE.md` §4a instead; a short pointer from `CLAUDE.md`'s existing timer section to that table would be a reasonable follow-up if the team wants one, but that's a call for a human to make, not to fold into this PR.
...
**Implementation status: F1/F3/F5/F6 done** (commits `6bb6d86`, `b152c8d`, `baf9f4e`, `de44dea`, `095e79d` on `claude/app-architecture-eval-pcfpj3`). `npm run lint` green throughout. **F2 and F7 are explicitly NOT implemented** — both are process/vendor/budget decisions for a human to make, not code (see "Open questions" below); this plan only surfaces them, per the original design.

This environment has no deployed Amplify sandbox backend or AWS credentials, so none of the new specs below have been executed — only written and structurally verified (`eslint`, `tsc -p tsconfig.test.json`, and `npx playwright test --list` against every project). **Running them for real (`npx ampx sandbox` + `npm run test:e2e:field-conditions`, or the `field-conditions-e2e` CI job via manual `workflow_dispatch`) is the necessary next step before trusting them as a release gate.**

- **F1**: new `field-conditions` Playwright project (`devices['iPhone 13']` — WebKit engine + mobile viewport in one), matched via an explicit `testMatch` list (not `full`'s `testIgnore`) against the three new specs below, so it can never silently absorb an unrelated spec. New `field-conditions-e2e` CI job, gated to `workflow_dispatch` only and deliberately excluded from `ci-policy-gate`'s required checks — a pre-release signal, not a merge blocker. `test:e2e:field-conditions` npm script; `e2e/README.md` and `README.md` updated.
- **Reuse-safety fix (not in the original plan, found during F3 implementation)**: the first attempt reused `full-workflow.spec.ts`'s setup/interaction helpers by exporting them and importing from the new spec files. `npx playwright test --list --project=field-conditions` then showed `full-workflow.spec.ts`'s own 2 tests too — Playwright registers `test()`/`test.describe()` calls as an import side effect, so importing a real spec file's exports leaks that file's tests into every project that imports it, silently defeating F1's whole "explicit testMatch" fix. Fixed by extracting `TEST_DATA` and the 10 reusable helpers into a new `e2e/game-workflow-helpers.ts` module containing no `test()` calls (see that file's header comment); `full-workflow.spec.ts` now imports them back, unchanged behavior. Re-verified with `--list` against `field-conditions`, `full`, and `smoke` — each shows only its own specs.
- **F3**: `e2e/offline-game-management.spec.ts` — offline substitution + offline halftime + reconnect, verified via the Reports page's per-player play-time totals that offline-created `PlayTimeRecord`s land on the correct boundaries (direct regression for Issue A's redesigned local-tracking-map fix); a second test verifies starting the second half while offline does not hard-block the coach (regression for the throw-based design Opus's plan review caught and the shipped fix avoids).
- **F5**: `e2e/concurrent-coaches.spec.ts` — two REAL, concurrently-running browser contexts (not team-sharing.spec.ts's sequential single-page identity swap) drive genuinely concurrent actions: one coach substitutes a player while the other records a goal, at the same time via `Promise.all()`. Verifies both changes converge with no lost write, from both coaches' own views — direct regression for Issue C's guard consolidation. The invite/accept flow is reimplemented locally (not imported from `team-sharing.spec.ts`) for the same reuse-safety reason as above.
- **F6**: `e2e/timer-gap-confirmation.spec.ts` — uses Playwright's Clock API (`page.clock.install` + `fastForward`, documented as "equivalent to user closing the laptop lid for a while and reopening it later") to simulate ~15 minutes of backgrounding while the game timer keeps running server-side. `lastStartTime` writes synchronously on game start (not on a periodic interval), so no timing dependency to wait on before installing the clock. The clock is installed at the `BrowserContext` level and persists across `page.reload()`, which forces the fresh `Game.observeQuery` snapshot `computeGapConfirmationDecision` needs to evaluate the gap. Two tests cover accept (timer resumes jumped forward) and decline (timer stays paused at the pre-gap time) — direct regression for Issue B.
- **F2 / F7 — open, for a human to decide** (unchanged from the original plan): F2 needs a device-farm vendor/budget decision; F7 needs a process owner for the field-beta cohort. Neither is implementable as code.
...
- **v5** — Issue A implemented and pushed (see its "Implementation status" note above), including a Major fix a `validation-reviewer` pass caught that the plan itself never anticipated (a second, un-patched offline-queue drain path). Issue B also implemented (see its "Implementation status" note): discovered and closed a false-positive bug in the v4 design itself during implementation — the gap-confirmation logic as designed would have fired on every normal "second coach opens an already-running game" case, not just genuine crash recovery, since nothing in v4 distinguished the two. Fixed with a new per-user/per-game `localStorage` continuity heartbeat not present in any prior version of this plan. Two rounds of `validation-reviewer` re-review each found a further real bug in the fix itself (a stale-closure `userId` that made the entire feature permanently inert, then a residual fallthrough reachable through a second path) — both fixed and verified.
- **v6 (this version)** — Issue C implemented (see its "Implementation status" note above): three pure decision functions extracted from `useGameSubscriptions.ts`'s `observeQuery` callback, preserving distinct inputs per the plan's explicit "do not collapse" guidance; the ordering-sensitive guard sequence left untouched; two characterization-test gaps the plan called out filled before touching any logic; guard inventory documented in `docs/ARCHITECTURE.md` §4a (not `CLAUDE.md`, per the plan's human-sign-off requirement). Sent to a `validation-reviewer` pass given this is the most fragile code in the app.
- **v4** — one of the two v3 reviews (Haiku) had significant accuracy problems and was discounted: it treated the plan (pre-implementation) as though the code should already reflect it, and made two factually wrong claims (that `UI-SPEC.md` was missing from Issue B's file list, and that a `setOffline()` helper existed in `e2e/helpers.ts` — neither true, both checked directly against the file/repo). The other review (Opus) found substantive, verified problems: **v2's Issue A fix was still incomplete** — a deterministic id alone doesn't get computed/enqueued at halftime for a record that was never visible locally or in DynamoDB, so the close still wouldn't happen; fixed by adding a local open-record tracking map that enqueues the close unconditionally, with the DB-scan path demoted to a cross-device backstop, and by fixing a silent-error-swallow in `useOfflineQueueDrain.ts` that would have dropped a failed replay of exactly this update. **v2's Issue B fix was in the wrong file** — the resume gap is absorbed in `useGameSubscriptions.ts` before `useGameTimer.ts` ever sees it — and its boundary-suppression rule was wrong for the second half (elapsed time is always past `halfLengthSeconds` once half 2 starts, which would have silently suppressed the modal for the entire second half); both corrected, and B now shares a file with Issue C's refactor, requiring explicit sequencing. **Issue F gained a real CI-wiring fix** (new specs would otherwise silently join the per-commit `full` lane; CI never installs WebKit) plus F5's shared-team fixture prerequisite and four app-specific real-world conditions (IndexedDB eviction, clock skew/DST, iOS low-power timer throttling, mid-game service-worker updates) that weren't in the original draft.
