...
- **v5 (this version)** — Issue A implemented and pushed (see its "Implementation status" note above), including a Major fix a `validation-reviewer` pass caught that the plan itself never anticipated (a second, un-patched offline-queue drain path). Issue B also implemented (see its "Implementation status" note): discovered and closed a false-positive bug in the v4 design itself during implementation — the gap-confirmation logic as designed would have fired on every normal "second coach opens an already-running game" case, not just genuine crash recovery, since nothing in v4 distinguished the two. Fixed with a new per-user/per-game `localStorage` continuity heartbeat not present in any prior version of this plan. Sent to a `validation-reviewer` pass; findings to be folded in once it reports.
