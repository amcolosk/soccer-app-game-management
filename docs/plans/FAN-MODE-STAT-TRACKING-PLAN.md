...
- **`GoalTracker` renders in three places in `GameManagement.tsx`, not one — the segmented control needs to go everywhere it does.** Verified: `GameManagement.tsx:2405` (in-progress tabpanel, inside `TabNav`'s "Goals" tab — this is the one the sketch above was written against), `:2562` (halftime tabpanel, same tab structure), and **`:2652` (the completed-game layout)** — this last one is not inside any tab at all; `.completed-layout` (`:2638`) renders `CompletedPlayTimeSummary` → `CompletedGameTimeline` → `GoalTracker` → `PreGameNotesPanel` as a flat stack of standalone sections. A segmented control scoped to "inside the Goals tab" literally cannot appear there. This matters concretely: Milestone B2's correction path for helper mistakes ("the coach corrects mistakes via the Milestone A `ShotSaveTracker`/existing `GoalTracker` edit UI") depends on the coach being able to reach Shots/Saves on a **completed** game, which is exactly when most of that review happens. Required: the segmented control (or, in the completed layout specifically, an equivalent same-pattern standalone section header, since there's no tab context to nest inside) must render at all three sites; the active-sub-view state should live on the shared props object both `GoalTracker` and `ShotSaveTracker` already take (`sharedGoalTrackerProps`, `GameManagement.tsx:2151`) rather than three independent local states that could drift out of sync per layout.
- `ShotSaveTracker.tsx` — same interaction shape as `GoalTracker.tsx`: modal-driven create, `GameActionRow`/`actionContract` for edit/delete, explicitly sets `loggedVia: 'COACH'` on every write. **Every entry starts with an Us/Opponent choice** — but concretely, that means *matching `GoalTracker.tsx`'s actual existing pattern*, not inventing a new one: `GoalTracker.tsx` doesn't ask inside a modal, it has two entry buttons (`.goal-buttons`, `GoalTracker.tsx:166-171`, styled at `App.css:3295`) — e.g. "Log Goal — Riverside Rovers" / "Log Goal — Lakeside FC" — and the modal that opens already knows which side was picked. `ShotSaveTracker.tsx` should mirror this exact two-button-per-sub-view shape (distinct from the public `StatTrackerView`'s own sheet-based per-tap question in Milestone B2, which is a different page with a different interaction model and stays as designed there). Selecting "Us" proceeds to our player picker as normal; selecting "Opponent" skips the player picker entirely and logs an aggregate event with no `playerId` — this app doesn't track an opposing roster, so an opponent shot/save is a count, not an attributed one. **Edit-modal semantics need a decision, not silence**: unlike `Goal`, neither `Shot` nor `Save` carries a `notes` field (`resource.ts:339` is `Goal`-only), so an opponent-attributed `Save` row has zero editable fields once created — decide whether to add `notes` to both for consistency, or suppress the Edit action (keep Delete) on opponent rows specifically. Also state explicitly whether an "Us" `Shot` requires a player (the way `Goal` requires a scorer, `GoalTracker.tsx:69-72,118-121`) or stays fully optional like `Save.playerId` — don't leave this to be improvised per-field during implementation.
...
- New tests for the segmented control (keyboard/ARIA nav between Goals/Shots/Saves sub-views) **at all three mount sites** — in-progress tab, halftime tab, and the completed layout — in `GameManagement.test.tsx` or a dedicated test file, whichever the existing Goals-tab test coverage pattern favors. The completed-layout case is the one most likely to be skipped by accident; don't skip it.
...
| `get-fan-game-view` | **guest** | Look up `ShareLink` by token; reject if missing/revoked/wrong type; query `listGamesByTeamId`, pick in-progress/halftime first else most recent; assemble `FanGameViewResult` via the indexed queries above. Rate-limited (see below). |
...
**Read-path rate limiting**: B2's `ShareLinkRateLimit` table doesn't exist yet at this point in the build order, and a public, fan-out read endpoint shouldn't ship without its own limiter in the meantime — add a same-shaped table now rather than waiting for B2:
...
Generous threshold (e.g. 30 reads/min) — this exists so an unauthenticated, fan-out read endpoint isn't a free billing-DoS vector for a leaked/screenshotted link, not to throttle normal polling.
...
### Open items to resolve in B1's own dedicated review round (recorded now, not fixed here)

- `get-fan-game-view` is specified to "reject if missing/revoked/**wrong type**" (see table above) — but Milestone B2's `StatTrackerView` is currently planned to poll this same query for its roster/game-state data. A `STAT_TRACKER` token is the wrong type for `getFanGameView` as written, so B2's page would never load against it. Decide during B1's review: either `getFanGameView` accepts both link types (gating what's in the payload on `type`), or B2 gets its own `getStatTrackerView` query. Whichever is chosen must also settle whether `playerId`s belong in the `FAN`-type payload at all (a pure fan viewer has no use for them) or only the `STAT_TRACKER` one.
...
**Current status**: Milestone A has been through a formal `architect-reviewer` + `ui-reviewer` round (cleared to `coding-agent` after the segmented-sub-view revision), and then a fourth round — two more independent full-plan validations run after the Us/Opponent addition — which surfaced one real blocker (the three-mount-site gap, now fixed above) plus the interaction-shape, edit-semantics, and file-list corrections also folded in. Because those changes are genuine UI decisions (the two-button-vs-sheet interaction shape, the opponent-row edit semantics, the completed-layout treatment) made *after* `ui-reviewer`'s last sign-off, **take one more `ui-reviewer` pass scoped to just Milestone A's revised Frontend section** before `coding-agent` starts — not a full re-review, just confirmation that these specific fixes read correctly. No further `architect-reviewer` round is needed for Milestone A. Milestones B1 and B2 have architecture-level findings folded in above (including two items each explicitly deferred to their own dedicated rounds — see "Open items to resolve in B1's own dedicated review round" and the opponent-path/roster-validation additions in B2) but have not yet had their own dedicated `architect-reviewer`/`ui-reviewer` passes — run those before implementing them, same as Milestone A got.
