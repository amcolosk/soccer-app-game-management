...
- **Rate-limit dimension parameter**, per the data-model change above.
- **Deterministic tiebreak among multiple "live" games.** `selectGameForFan`'s live branch currently does `games.find(g => g.status === 'in-progress' || g.status === 'halftime')` against an unordered GSI query result. Nothing in this app auto-completes a game — `status` stays `in-progress` indefinitely until a coach explicitly ends it — so a team with one abandoned/never-completed game *and* today's actual live game has two matches, and the `find()` winner is arbitrary per invocation. For B1 (read-only) this was a cosmetic wrong-scoreboard risk; for B2 it's a silent-data-corruption risk with no undo: a helper's taps could write `Goal`/`Shot`/`Save` rows onto last month's abandoned game, never show up on the coach's actual live screen (so nobody notices mid-game), and there's no way for the helper to tell. Fix: among live candidates, prefer the most recent `gameDate`, then `updatedAt`, as the tiebreak. `submit-stat-event` (below) adds its own belt-and-suspenders check on top of this.
...
**Required before the rest of B2 is built on this mechanism — a validation spike, not an assumption:**
1. Build a minimal sandbox spike: one IAM-authed `generateClient<Schema>({ authMode: 'iam' })` call from a throwaway/test Lambda, creating a `Goal` (or a disposable test model) with a `coaches` array, and confirm — against a real deployed sandbox, not just a local synth — that (a) the record persists with `coaches` exactly as supplied (owner/dynamic-group-authorized fields can have surprising auto-population behavior under `ownersDefinedIn`-style rules that a raw IAM caller might not trigger the same way a Cognito-authenticated one does), and (b) a coach's `observeQuery` subscription actually receives the create event, with `coaches` present in the delivered payload (not just in the stored record) so `ownersDefinedIn` filtering actually delivers it.
2. **Named fallback if the spike fails**: drop to a SigV4-signed `fetch` directly against the AppSync GraphQL endpoint (lower-dependency than the full Amplify Data client, same underlying effect — a real GraphQL mutation, so the subscription still fires) as the first fallback; raw `DynamoDBDocumentClient`/`PutCommand` (accepting the GSI-propagation-style lag this repo already accepts elsewhere, plus a stated client-side polling workaround on the coach's screen to compensate for the lost subscription) as the last resort if even that doesn't pan out. Do not silently fall back without recording which path was actually used and why.
3. **Full wiring, once the spike confirms the approach** (name every piece — the original draft's version omitted enough that a literal implementation wouldn't run): the Lambda's `resource.ts` needs `resourceGroupName: 'data'` (every function in this repo that touches data sets this — see `get-fan-game-view/resource.ts` — it's the documented mitigation for a circular dependency between the function and the data stack that a handler-type function would otherwise hit). Inside the handler, use `getAmplifyDataClientConfig(env)` from `@aws-amplify/backend/function/runtime` to obtain both `resourceConfig` and `libraryOptions`, and call `Amplify.configure(resourceConfig, libraryOptions)` with both — not just the GraphQL endpoint URL alone, which is insufficient to produce a working, credentialed client. In `amplify/backend.ts`, grant `allow.resource(submitStatEvent).to(['create'])` on `Goal`/`Shot`/`Save` — **scoped to `create` only**, not an unscoped `allow.resource()`, which would otherwise give this public-write Lambda's role delete/update access over every team's stat rows with team-scoping enforced only in handler code, not IAM. Note explicitly in the code (and in the ARCHITECTURE.md doc entry below) that this grant is IAM-role-scoped, not team-scoped — the Lambda's own handler logic is the only thing preventing it from writing to a different team's game than the one its token resolved to, which is exactly why the tiebreak/game-selection fix above and the roster-membership validation matter as much as they do here.
...
- The `allow.resource()` grant on `Goal`/`Shot`/`Save` is IAM-role-scoped, not team-scoped (see the wiring section above) — this is an accepted, necessary tradeoff of the AppSync-write fix, flagged explicitly for security-reviewer's attention rather than left implicit.

### Tests

- **The validation spike itself** is a required, recorded step (not a unit test, but a gating action) — see "Required before the rest of B2 is built on this mechanism" above. Document its outcome (which write mechanism was actually used) before the rest of this milestone's implementation proceeds.
- `submit-stat-event` handler tests: token validation, both rate-limit dimensions (write-prefixed), `GAME_NOT_LIVE` gating (including against the specific tiebroken game, not just "any live game exists"), server-derived `gameSeconds`/`half` correctness (including the paused-within-a-half and halftime cases), `eventType` allowlist rejection, explicit assertion that `coaches[]` is populated from the team's current array (including for opponent-side events), rejection when `forUs === false` but a `playerId`/`assistPlayerId` is supplied, rejection when `forUs === true` and `playerId`/`assistPlayerId` doesn't belong to the token's team roster or when `assistPlayerId === playerId`, and the `SubmitStatEventResult.reason` value for each rejection path.
...
- `shareLinkAccess.ts` — new tests for the rate-limit dimension parameter and the live-game tiebreak fix.
...
- The validation spike (above) must complete, with its outcome recorded, before B2's implementation proceeds past the write-path mechanism.
...
**Milestone B2 has now completed its own dedicated `architect-reviewer` + `ui-reviewer` round** (Round 6 above) and this revision folds in every finding from it. Unlike A and B1, this round's most significant finding isn't a plan correction to fold in and move past — it's a **required gating step for implementation itself**: a small validation spike (see B2's "Required before the rest of B2 is built on this mechanism" section) must run and its outcome must be recorded *before* `coding-agent` builds the rest of the milestone on top of the AppSync-write mechanism, because that mechanism has zero precedent in this repo and was previously evaluated and rejected here for exactly that reason. Once the spike's outcome is known (and, if it doesn't pan out, the named fallback is chosen instead), this milestone is otherwise ready for `coding-agent` — no further `architect-reviewer`/`ui-reviewer` round is expected to be needed unless the spike's result changes the write-path design materially enough to warrant one.
