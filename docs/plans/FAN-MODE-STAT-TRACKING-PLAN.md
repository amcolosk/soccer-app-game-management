...
4. Else (no games with a date at all, or the recency window in (2) has lapsed with nothing upcoming), fall back to creation order and render the generic "team hasn't started a game yet" state.
...
- `FanGameView.test.tsx` — all five named states (revoked, next-game, finished-with-date, no-game-yet, rate-limited); polling under fake timers, pause-on-hidden, and immediate re-poll on visibility-resume.
