...
Unlike A and B1, this milestone's single most significant remaining item isn't a plan correction to fold in and move past — it's a **required gating step for implementation itself**: a small validation spike (see B2's "Required before the rest of B2 is built on this mechanism" section) must run and its outcome must be recorded *before* `coding-agent` builds the rest of the milestone on top of the AppSync-write mechanism, because that mechanism has zero precedent in this repo and was previously evaluated and rejected here for exactly that reason. The spike is orthogonal to everything else in this round's findings and may run immediately. Once its outcome is known (and, if it doesn't pan out, the named fallback is chosen instead), this milestone is ready for `coding-agent` — no further `architect-reviewer`/`ui-reviewer` round is expected to be needed unless the spike's result changes the write-path design materially enough to warrant one.
