...
- `ShotSaveTracker.tsx` — same interaction shape as `GoalTracker.tsx`: modal-driven create, `GameActionRow`/`actionContract` for edit/delete, explicitly sets `loggedVia: 'COACH'` on every write.
