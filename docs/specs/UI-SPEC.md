...
- Tapping a target opens a bottom sheet, driven by a shared per-tap flow:
  ...
  2. **Us path** → player picker (scorer for Goal, shooter for Shot, keeper for Save), with an explicit **"Skip / unknown player"** affordance (every event type's `playerId` is optional in the mutation) → for Goal, a second player-picker step for an optional assist ("No assist" advances with none selected) → for Shot, an on-target/off-target step → confirm/submit. Every player picker (scorer/shooter/keeper and the assist step alike) lists on-field players first, then bench players, under **"On the field"**/**"Bench"** group labels shown only when both groups are non-empty (an all-bench roster, e.g. the game isn't in-progress, gets no labels). Each player button is now labeled `#{jersey number or "?"} {first name} {last name}` — the same number→name matching a helper does on the "On the Field" pitch view carries through to the screen where the tap that matters actually happens. **Save-specific carve-out (Save Auto-Goalkeeper Attribution):** when `StatTrackerViewResult.activeGoalkeeperId` names a player present in `roster`, a "Us" Save skips the full player picker entirely and lands on a `confirmKeeper` step instead — "*{name} made the save?*" with a primary **"Yes, log it"** (submits immediately, reusing the already-set `playerId`) and a de-emphasized **"Not right? Pick another keeper"** escape hatch that drops into the same full player picker described above (so "Skip / unknown player" is still reachable, just one tap deeper via the override branch). When `activeGoalkeeperId` is absent, ambiguous, or names a player not in `roster`, the Save flow is unchanged from the player-picker behavior above.
