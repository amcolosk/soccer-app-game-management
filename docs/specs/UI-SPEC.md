...
- **On the Field** (`<h2>`, only rendered while the tap UI is unlocked, i.e. in-progress): the active roster's on-field players and their position names, derived from each player's open `PlayTimeRecord` → `FormationPosition` lookup — same `FormationPosition`-not-`FieldPosition` source as `activeGoalkeeperId` below. Empty ("No lineup data yet.") rather than hidden when in-progress but no open records resolve.
...
#### Accessibility
- Every state renders exactly one page-level `<h1>` before any `<h2>`s, matching §7.14 — the live in-progress state's "On the Field" `<h2>` follows the `<h1>` header, same ordering
...
#### Responsive behavior
Same as §7.14: phone (< 768px) full-width single column; tablet+ (≥ 768px) content max-width `640px`, centered. The tap grid wraps (`flex-wrap: wrap`) rather than overflowing at narrow widths, and the bottom sheet stays full-width up to the same `640px` cap. The scoreboard/clock row, "On the Field" lineup section, and upcoming-games list are plain stacked content within that same single column — no separate breakpoint behavior of their own.
