...
- **No credentials in the repository** — AWS secrets are never stored in code or `amplify_outputs.json`; they are fetched at runtime from S3 via OIDC.
...
| `pull_request` — same-repo, `run-smoke-e2e` label **or** risk paths changed | Yes | Yes | Smoke | `quality` + `smoke-e2e` pass |
...
| `smoke-e2e` | S3 object / S3 URI | Fetched at runtime via OIDC → S3; written to `amplify_outputs.json`; deleted in `if: always()` cleanup step |
| `full-e2e` | S3 object / S3 URI | Fetched at runtime via OIDC → S3; written to `amplify_outputs.json`; deleted in `if: always()` cleanup step |
...
1. `smoke-e2e` and `full-e2e` request `id-token: write` at the job level (not at workflow level).
...
3. The job reads `vars.AMPLIFY_OUTPUTS_S3_URI` and aborts if the variable is empty.
4. `aws s3 cp "$AMPLIFY_OUTPUTS_S3_URI" amplify_outputs.json` downloads the runtime config to `amplify_outputs.json`.
...
| `AWS_REGION` | Variable | AWS region (defaults to `us-east-2` if unset) |
| `AMPLIFY_OUTPUTS_S3_URI` | Variable | Full S3 URI, e.g. `s3://teamtrack-ci-config/e2e/amplify_outputs.json` |
...
| Action | Resource |
|--------|---------|
| `s3:GetObject` (allow) | `arn:aws:s3:::teamtrack-ci-config/e2e/amplify_outputs.json` |
...
| Force full E2E on a specific PR | Add the `run-full-e2e` label to the PR (same-repo only). `full-e2e` runs with S3-backed config. |
| S3 config object is outdated (stale `amplify_outputs.json`) | Upload an updated object to the configured S3 URI, e.g. `aws s3 cp amplify_outputs.json s3://teamtrack-ci-config/e2e/amplify_outputs.json`. Re-run the failed workflow. |
